...
//             navigation.navigate("OnboardingParent", { userType: "parent" });
//           } else if (selectedValue === "child") {
//             navigation.navigate("OnboardingChild", { userType: "child" });
//           }
//         }}
...
import React, { useState } from "react";
import { View, Text, TouchableOpacity } from "react-native";
import tw from "../../tw"; // Tailwind styling
import { Picker } from '@react-native-picker/picker';

const OnboardingScreen = ({ onUserTypeSelect }) => {
  const [selectedValue, setSelectedValue] = useState(""); // Holds the selected user type
  const [isFocused, setIsFocused] = useState(false); // For picker focus
  ...
  return (
    <View style={tw`flex-1 justify-center items-center bg-white`}>
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          if (selectedValue) {
            onUserTypeSelect(selectedValue); // Call the function passed as a prop
          }
        }}
