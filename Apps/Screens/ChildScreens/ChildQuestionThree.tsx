...
const ChildQuestionThree = () => {
  ...
  const { progress: passedProgress } = route.params || { progress: 0 }; // Get the passed progress value or default to 0

  const [selectedAnswer, setSelectedAnswer] = useState(null);
  const [progress, setProgress] = useState(passedProgress); // Set initial progress based on passed value
  ...
  const handleAnswerSelection = (index) => {
    ...
    setTimeout(() => {
        navigation.navigate('ChildCongrats', { progress: newProgress }); // Pass progress to the next screen
      }, 1000);
  };
